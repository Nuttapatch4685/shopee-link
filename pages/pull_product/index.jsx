...
const DailyProduct = () => {
  ...
  const extractLinks = (hasCommission) => {
    ...
    items.forEach((item) => {
      // ✅ รองรับทุกเวอร์ชันของ Shopee
      const commissionImg = item.querySelector(
        'img[alt="ams-label"], img[src*="ams-label"], img[src*="sponsored"], img[alt="shopee-partner"], img[alt*="promotion-label"], img[alt*="promotion-label-icon"], img[alt*="flag-label"], img[src*="promotion-label"], img[src*="flag-label"]'
      );
      const isCommission = commissionImg !== null;

      if (isCommission === hasCommission) {
        const salesText = item.textContent || "";
        // ✅ Regex รองรับทั้ง "+", "พัน", "/เดือน"
